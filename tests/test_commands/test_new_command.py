...
def mock_template_zip_file():
    ...
    Possible test would be to actually replicate a real template in memory and not just use random files  # noqa: E501
...
class TestMainCli(fake_filesystem_unittest.TestCase):
    ...
    def test_not_existing_template_exception_thrown(self):
        ...
        template_retriever.get_templates = MagicMock(return_value={"default": None})
        command_executor = NewCommandExecutor(template_retriever=template_retriever)

        # Act + Assert
        self.assertRaises(
            Exception, command_executor.new, "nut_shell", "NOT_EXISTING_TEMPLATE"
        )
    ...
    def test_shows_informative_message_when_offline(self):
        ...
        self.assertRaisesRegexp(
            UsageError, "offline", command_executor.new, "nut_shell", "base", "master"
        )
    ...
    def test_not_existing_local_template_dir_thrown(self):
        ...
        self.assertRaisesRegexp(
            ...
            "new_shell",
            "local:{template_dir}".format(template_dir="shell_template_root"),
            "master",
        )

    @patch(
        "shellfoundry.commands.new_command.NewCommandExecutor._verify_template_standards_compatibility"  # noqa: E501
    )
    def test_cookiecutter_called_for_existing_template(self, verification):
        ...
        template_retriever.get_templates = MagicMock(
            return_value={
                "base": [
                    ShellTemplate("base", "", "https://fakegithub.com/user/repo", "7.0")
                ]
            }
        )
    ...
    def test_can_generate_local_template_into_same_directory(self, verification):
        ...
        command_executor._get_template_params = MagicMock(return_value={})
    ...
    def test_integration_can_generate_shell_from_specific_version(self, verification):
        ...
        templates = {
            "tosca/resource/test": [
                ShellTemplate("test-resource", "", "url/test", "8.1", "resource")
            ]
        }
        ...
        httpretty.register_uri(
            ...
            content_disposition="attachment; filename=quali-resource-test-dd2ba19.zip",
            stream=True,
        )
        template_compiler = MagicMock()
        ...
        # Act
        with patch.object(
            TemplateRetriever, "get_templates", return_value=templates
        ), patch(
            "shellfoundry.utilities.template_url._parse_repo_url",
            return_value=repo_info,
        ), patch.object(
            TempDirContext,
            "__enter__",
            return_value=self.fs.CreateDirectory("mock_temp").name,
        ):
            command_executor = NewCommandExecutor(
    ...
    @httpretty.activate
    def test_integration_can_generate_gen1_shell_from_specific_version(
        self, verification
    ):
        # Arrange
        templates = {
        ...
        httpretty.register_uri(
            httpretty.GET,
            "https://api.github.com/repos/quali/resource-test/zipball",
            body=zipfile.read(),
            content_type="application/zip",
            content_disposition="attachment; filename=quali-resource-test-dd2ba19.zip",
            stream=True,
    ...
    def test_fail_to_generate_shell_when_requested_version_does_not_exists(
        ...
        with patch.object(
            ...
            TemplateVersions, "get_versions_of_template", return_value=template_versions
        ), self.assertRaises(
            BadParameter
        ) as context:
            command_executor = NewCommandExecutor(
        ...
        # Assert
        self.assertTrue(
            "Requested standard version ('1.1') does not match template version. \n"
    ...
    @httpretty.activate
    def test_new_cmd_creates_gen2_in_latest_version_that_matches_the_standard_version_on_cs(  # noqa: E501
        self, verification
    ):
        # Arrange
        templates = """templates:
    ...
    - name : gen2/networking/switch
      ...
      min_cs_ver: 8.0"""
        ...
        httpretty.register_uri(
            httpretty.GET,
            "https://api.github.com/repos/QualiSystems/shellfoundry-tosca-networking-template/zipball/5.0.1",  # noqa: E501
            body=zipfile.read(),
            content_type="application/zip",
            content_disposition="attachment; filename=quali-resource-test-dd2ba19.zip",
            stream=True,
        ...
        # Act
        with patch.object(
            TemplateRetriever, "_get_templates_from_github", return_value=templates
        ), patch.object(
            TemplateRetriever, "_get_min_cs_version", return_value=None
        ), patch.object(
            TempDirContext,
            "__enter__",
            return_value=self.fs.CreateDirectory("mock_temp").name,
    ...
    @patch(
        "shellfoundry.commands.new_command.NewCommandExecutor._verify_template_standards_compatibility"
    )
    @httpretty.activate
    def test_new_cmd_creates_gen2_when_get_cs_standards_feature_is_unavailable(
        ...
        # Act
        with patch.object(
            Standards, "_fetch_from_cloudshell", side_effect=FeatureUnavailable()
        ), patch.object(
            TempDirContext,
            "__enter__",
            return_value=self.fs.CreateDirectory("mock_temp").name,
    ...
    def test_requested_template_does_not_exists_raises_an_error(self, verification):
        ...
        with patch.object(
            ...
            # Assert
            output_msg = (
                "Template gen2/doesnot/exists does not exist. Supported templates are: gen1/resource, "
            ...
            self.assertRaisesRegexp(
                BadParameter, output_msg, cmd.new, "new_shell", "gen2/doesnot/exists"
            )
    ...
    def test_new_command_with_invalid_shell_name_raises_bad_parameter_error(self):
        ...
        # Assert
        output_msg = u"Shell name must begin with a letter and contain only alpha-numeric characters and spaces."  # noqa: E501
        self.assertRaisesRegexp(
            BadParameter, output_msg, cmd.new, shell_name, "gen2/resource"
        )
